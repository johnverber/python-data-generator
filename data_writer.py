...
#no arguments ask for number of data
	...
	while True:
		try:
			answer = True
			num = input("Please enter a number between 1-1000000: ")
			num = int(num)
			...
			print("Your number is too large, must be less than 1000000! Please try again...")
...
for x in range(0, num):
	...
	if rand_gender == 1:
		first = random.choice(girl_list).strip()
		last = random.choice(last_list).strip()
		email = first + '.' + last + ext[random.randint(0,3)].strip()
		address = random.choice(street_list).strip()
		ph_num = random.choice(phone_list).strip()
		data.write(first + ',' + last + ',' + email + ',' + address + ',' + ph_num + '\n')
	elif rand_gender == 0:
		first = random.choice(boy_list).strip()
		last = random.choice(last_list).strip()
		email = first + '.' + last + ext[random.randint(0,3)].strip()
		address = random.choice(street_list).strip()
		ph_num = random.choice(phone_list).strip()
		data.write(first + ',' + last + ',' + email + ',' + address + ',' + ph_num.strip() + '\n')
